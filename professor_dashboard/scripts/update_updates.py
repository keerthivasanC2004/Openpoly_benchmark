...
DEFAULT_RESEARCHERS = ROOT / "data" / "researchers.json"
...
def json_request(path: str, params: dict[str, Any], retries: int = 2, timeout: int = 15) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        url,
        headers={"Accept": "application/json", "User-Agent": f"PolymerAIRadar/1.0 (mailto:{POLITE_EMAIL})"},
    )
...
def candidate_score(researcher: dict[str, Any], candidate: dict[str, Any]) -> int:
    target = normalize(researcher.get("name"))
    display = normalize(candidate.get("display_name"))
    score = 0
    if display == target:
        score += 12
    elif target and (target in display or display in target):
        score += 7
    target_parts, display_parts = set(target.split()), set(display.split())
    score += len(target_parts & display_parts) * 2

    tokens = [normalize(x) for x in researcher.get("institution_tokens", [])]
    institutions: list[str] = []
    for affiliation in candidate.get("affiliations") or []:
        for institution in affiliation.get("institution") and [affiliation["institution"]] or []:
            institutions.append(normalize(institution.get("display_name")))
    last_known = candidate.get("last_known_institutions") or []
    institutions.extend(normalize(x.get("display_name")) for x in last_known)
    for token in tokens:
        if token and any(token in institution for institution in institutions):
            score += 5
...
def fetch_researcher_updates(researcher: dict[str, Any], since: str, per_author: int) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    ...
    for work in work_payload.get("results") or []:
        ...
        selected.append({
            ...
            "author": author_label(work) or researcher["name"],
            ...
            "url": work_url(work),
            "relevance": f"Automated polymer/AI relevance score: {score_value}",
            "confidence": "high" if score >= 15 else "medium",
            "openalex_author_id": author_id,
            "openalex_work_id": str(work.get("id", "")).rsplit("/", 1)[-1],
        })
        if len(selected) >= per_author:
            break
...
def build_feed(researchers_path: Path, output_path: Path, lookback_days: int, per_author: int, max_items: int) -> dict[str, Any]:
    ...
    # Fail fast when OpenAlex is unavailable. This prevents a network outage from
    # multiplying into dozens of per-researcher timeouts during a Pages build.
    try:
        json_request("/works", {"per-page": 1}, retries=1, timeout=10)
        ...
        payload = {
            ...
            "status": "fallback" if previous.get("items") else "degraded",
            "source": "OpenAlex API",
    ...
    for researcher in researchers:
        try:
            found, unresolved_item = fetch_researcher_updates(researcher, since, per_author)
            items.extend(found)
            if unresolved_item:
                unresolved.append(unresolved_item)
        except RuntimeError as exc:
            failures.append(f"{researcher.get('name')}: {exc}")
        time.sleep(0.08)
    ...
    for item in items:
        ...
        if not existing or item.get("confidence") == "high":
            unique[key] = item
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--researchers", type=Path, default=DEFAULT_RESEARCHERS)
...
def main() -> int:
    args = parse_args()
    if args.lookback_days < 1 or args.per_author < 1 or args.max_items < 1:
    ...
    print(f"Wrote {len(payload['items'])} updates to {args.output} ({payload['status']}).")
    return 0
